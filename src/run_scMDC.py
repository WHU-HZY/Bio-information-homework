...
if __name__ == "__main__":
    ...
    parser.add_argument('--cutoff', default=0.4, type=float, help='Start to train combined layer after what ratio of epoch')
    parser.add_argument('--batch_size', default=512, type=int)
    parser.add_argument('--data_file', default='../datasets')
    parser.add_argument('--maxiter', default=100, type=int)
    ###
    parser.add_argument('--pretrain_epochs', default=90, type=int)
    ###
    ...
    parser.add_argument('--sigma1', default=2, type=float)
    ...
    root = args.data_file
    x1 = pd.read_csv(root + '/scRNA_seq_for_final.tsv', sep='\t', index_col=0).T.values.astype(np.float32)
    x2 = pd.read_csv(root + '/scATAC_seq_for_final.tsv', sep='\t', index_col=0).T.values.astype(np.float32)
    y = pd.read_csv(root + '/label_for_final.tsv', sep='\t',index_col=0).values.flatten()
    ...
    adata2.obs['Group'] = y
    adata2.obs['factor_size'] = 1.0
    adata2 = normalize(adata2,
                      size_factors=False,
                      normalize_input=True,
                      logtrans_input=False)
    adata2.raw = adata2
    print(adata1.X.dtype, adata2.X.dtype)
    ...
    print(args, file=open(args.save_dir + "/" + str(args.run) + "_config.csv", "w"))
